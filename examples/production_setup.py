...
def production_logger_setup():
    ...
    app_log_writer = FileWriter(log_dir / "app.log", min_level=LogLevel.INFO)

    error_log_writer = FileWriter(log_dir / "error.log", min_level=LogLevel.ERROR)
    ...
    multi_writer = MultiWriter(console_writer, buffered_app_writer, error_log_writer)

    # Create production logger
    logger = Logger(writer=multi_writer, min_level=log_level).with_context(
...
def microservice_logger():
    ...
    service_logger = base_logger.with_context(
        service="user-service", version="2.1.0", deployment="k8s-cluster-1"
    )
...
def error_handling_setup():
    ...
    error_multi_writer = MultiWriter(
        ConsoleWriter(use_colors=True), error_log_writer, alert_writer
    )
...
def log_filtering_setup():
    ...
    filtered_logger.info(
        "User login", user_id="user-123", sensitive=True, operation="password_change"
    )
